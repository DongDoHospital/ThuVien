window.onload = function () {
	...
	// Số lượng hình banner
	var linkimg = "img/banners/banner1.png";
	addBanner(linkimg);
	...
	var tags = ["Test lấy da", "Khử khuẩn"];
	for (var t of tags) addTags(t, "index.html?search=" + t);

	// Thêm danh sách hãng điện thoại
	var company = ["Daotao.jpg", "Kehoach.jpg", "Qdbv.jpg", "Thongbao.jpg"];
...
var soLuongSanPhamMaxTrongMotTrang = 25;
...
function addKhungSanPham(tenKhung, color, filter, len, ele) {
	...
	s += `	</div>
			<a class="xemTatCa" href="index.html?` + filter.join('&') + `" style="` + borderA + `">
				Xem tất cả ` + spResult.length + ` bài viết
			</a>
		</div> <hr>`;
...
function addBanner(img, link) {
	var newDiv = `<div class='item'>
						<a target='_blank' href=` + link + `>
							<img src=` + img + ` style="width: 1205px; height: 292.3px">
						</a>
					</div>`;
